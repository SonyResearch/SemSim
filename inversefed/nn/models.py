...
def construct_model(model, num_classes=10, seed=None, num_channels=3, modelkey=None, pretrained=False):
    ...
    if model in ['ConvNet', 'ConvNet64']:
        ...
        model = ResNet(torchvision.models.resnet.BasicBlock, [2, 2, 2, 2], num_classes=num_classes,num_channels=num_channels, base_width=64)
    elif model == 'ResNet18_tv':
        model = torchvision.models.resnet18()
        model.fc = nn.Linear(512, num_classes)
    elif model == 'ResNet34':
        model = ResNet(torchvision.models.resnet.BasicBlock, [3, 4, 6, 3], num_classes=num_classes, num_channels=num_channels, base_width=64)
    elif model == 'ResNet50':
        if pretrained:
            model = ResNet50(torchvision.models.resnet.Bottleneck, [3, 4, 6, 3], num_classes=num_classes, num_channels=num_channels, base_width=64)
        else:
            model = ResNet(torchvision.models.resnet.Bottleneck, [3, 4, 6, 3], num_classes=num_classes, num_channels=num_channels, base_width=64)
...
class ConvNet(torch.nn.Module):
    ...
    def __init__(self, width=32, num_classes=10, num_channels=3):
        ...
        self.model = torch.nn.Sequential(OrderedDict([
            ...
            ('flatten', torch.nn.Flatten()),
            ('linear', torch.nn.Linear(36 * width, num_classes))  #36 * width,
        ]))
