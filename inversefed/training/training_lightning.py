...
import numpy as np
from inversefed.data.loss import  Classification, TripletLoss, TripletLoss_PSNR
...
class LitModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if isinstance(self.loss_fn, torch.nn.BCEWithLogitsLoss):
            ...
            # anchor_embed = self.model(data[0])
            # positive_embed = self.model(data[1])
            # negative_embed = self.model(data[2])
            # acc = 0
            #
            # anchor_embed = F.normalize(anchor_embed, p=2, dim=1)
            # positive_embed = F.normalize(positive_embed, p=2, dim=1)
            # negative_embed = F.normalize(negative_embed, p=2, dim=1)
            #
            # loss = self.loss_fn(anchor_embed, positive_embed, negative_embed)

            anchor_embed = self.model(data[0])
            ori_embed = self.model(data[1])
            positive_embed = self.model(data[2])
            negative_embed = self.model(data[3])

            anchor_embed = F.normalize(anchor_embed, p=2, dim=1)
            ori_embed = F.normalize(ori_embed, p=2, dim=1)
            positive_embed = F.normalize(positive_embed, p=2, dim=1)
            negative_embed = F.normalize(negative_embed, p=2, dim=1)
            neg_pos = label[1]
            acc = 0
            loss = self.loss_fn(anchor_embed, ori_embed, positive_embed, negative_embed, neg_pos)
        elif isinstance(self.loss_fn, TripletLoss_PSNR):
            data, psnr = batch
            # anchors, positives, negatives = batch
            anchor_embed = self.model(data[0])
            positive_embed = self.model(data[1])
            negative_embed = self.model(data[2])
            positive_embed1 = self.model(data[3])
            negative_embed1 = self.model(data[4])

            anchor_embed = F.normalize(self.model(data[0]))
            positive_embed = F.normalize(self.model(data[1]))
            negative_embed = F.normalize(self.model(data[2]))
            positive_embed1 = F.normalize(self.model(data[3]))
            negative_embed1 = F.normalize(self.model(data[4]))
            acc = 0
            loss = self.loss_fn(anchor_embed, positive_embed, negative_embed, positive_embed1, negative_embed1, psnr)
        else: 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if isinstance(self.loss_fn, torch.nn.BCEWithLogitsLoss):
            ...
            # acc = 0
            # loss = self.loss_fn(anchor_embed, positive_embed, negative_embed)
            # (img, ori_img, positive_img, negative_img), torch.tensor([cls_idx, neg_pog])
...
def train_pl(model, loss_fn, trainloader, validloader, defs, setup=dict(dtype=torch.float, device=torch.device('cpu')), save_dir=None, opt=None):
    ...
    if len(os.listdir(save_dir)) > 0 and glob.glob(save_dir+'/*.ckpt'):
        trainer.fit(model, 
                trainloader,
                validloader,
